...
import UserErrorNames from 'models/users/errors';
...
describe('AuthService', () => {
  ...
  describe('Validate user', () => {
    it('should validate if user and password is correct exists', async () => {
      ...
      await expect(authService.validateUser(userEmail, userPassword, clientId)).resolves.toEqual(
        rest,
      );
      expect(userService.findPasswordByEmail).toBeCalledTimes(1);
      expect(userService.findPasswordByEmail).toBeCalledWith(userEmail, clientId);
    });
    ...
    it('should throw an error if user does not exists', async () => {
      ...
      await expect(authService.validateUser(falseEmail, userPassword, clientId)).rejects.toEqual(
        AuthErrorNames.INCORRECT_USER_PASSWORD,
      );
    ...
    it('should throw an error if password does not exits', async () => {
      ...
      await expect(authService.validateUser(userEmail, falsePassword, clientId)).rejects.toEqual(
        AuthErrorNames.INCORRECT_PASSWORD,
      );
      expect(userService.findPasswordByEmail).toBeCalledTimes(1);
      expect(userService.findPasswordByEmail).toBeCalledWith(userEmail, clientId);
    });
  ...
  describe('user registration', () => {
    test('should register new user', async () => {
      ...
      await expect(authService.register(userToBeSaved, clientId)).resolves.toEqual(userData);

      expect(userService.saveUserRegistration).toHaveBeenCalledTimes(1);
      expect(userService.saveUserRegistration).toHaveBeenCalledWith(userToBeSaved);
    });
    ...
    test('should throw an error when registering', async () => {
      ...
      const userToBeSaved = { email: userEmail, password: userPassword };
      await expect(authService.register(userToBeSaved, clientId)).rejects.toEqual(
        AuthErrorNames.USER_REGISTRATION,
      );
