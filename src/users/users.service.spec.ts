...
describe('UsersService', () => {
      ...
      providers: [
        UsersService,
        {
          ...
          useValue: {
            ...
            findByRestorePasswordToken: jest
              .fn()
              .mockResolvedValue(userDataRestoreToken),
          },
        },
      ],
    }).compile();
  ...
  describe('Update user', () => {
    it('should update user with new values', async () => {
      await expect(
        userService.update(userId, updateUserValue),
      ).resolves.toBeTruthy();
  ...
  describe('Find user data with password', () => {
    it('returns user data with password', async () => {
      ...
      ).resolves.toEqual(userWithPassword);
      expect(userRepository.findDelicateUserData).toHaveBeenCalledTimes(1);
      expect(userRepository.findDelicateUserData).toHaveBeenCalledWith(
        userEmail,
        clientId,
      );
    });
  ...
  describe('Find user with email', () => {
    it('Find user data without password', async () => {
      ...
      expect(userRepository.findUserData).toHaveBeenCalledWith(
        userEmail,
        clientId,
      );
    });
  ...
  describe('Find user by restore password token', () => {
    it('Find user data successfully', async () => {
      ...
      ).resolves.toEqual(userDataRestoreToken);
      expect(userRepository.findByRestorePasswordToken).toHaveBeenCalledTimes(
        1,
      );
  ...
  describe('Saves restore password token', () => {
    it('Saves restore password token successfully', async () => {
      await expect(
        userService.saveRestorePasswordToken(
